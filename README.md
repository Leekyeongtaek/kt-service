...
## 5. AI 회고 활용
```text
1.이 코드가 무엇을 하는 코드인지 짧게 설명
[API에 대한 설명]
2. 리뷰 포인트
    1.설계 및 대안 탐색 (Alternative Approaches)
    "이 기능을 구현하는 다른 디자인 패턴이나 접근 방식이 있을까? 각각의 장단점을 비교해줘"
    2.코드의 견고함 (Robustness & Edge Cases)
    "이 방식이 해당 언어/프레임워크의 'Best Practice' 혹은 'Idiomatic'한 방식이야? 아니라면 왜 그래?"
    "코드에 포함된 함수(또는 라이브러리)의 공식 문서 링크나 관련 가이드를 찾을 수 있는 키워드를 알려줘."
    "코드에 포함된 기술의 도입된 배경이나, 이 라이브러리가 해결하려고 했던 핵심 문제가 뭐야?"
    3.시니어의 시선 (Senior's Perspective')
    "이 코드에서 발생할 수 있는 잠재적인 에러 상황 또는 보안 측면에서 취약한 부분이 있을까?"
    4.클린코드 또는 이펙티브 자바에 있는 기준과 비교
    +만약 굳이 수정할 부분이 없다면 없다고 솔직하게 말해줘.
